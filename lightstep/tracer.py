...
def init_tracer(*args, **kwargs):
    ...
    :param int periodic_flush_seconds: seconds between periodic flushes, or 0
        to disable
    """
    return BasicTracer(reporter_module.LightStepReporter(*args, **kwargs))
...
def init_debug_tracer():
    ...
    LightStep."""
    return BasicTracer(reporter_module.LoggingReporter())
...
class BasicTracer(Tracer):
    ...
    def __init__(self, reporter=None, sampler=None):
        self.reporter = NullReporter() if reporter is None else reporter
    ...
    def _report_span(self, span):
        if isinstance(self.reporter, reporter_module.LoggingReporter):
            self.reporter.report_span(span)
        elif isinstance(self.reporter, reporter_module.LightStepReporter): 
            self.reporter.record_span(span)
